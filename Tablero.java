...
public class Tablero {
    ...
    public static String[][] casillaPortal(char tipoCasilla[],String jugadores[][],int turno) {
        int cantidadPortal = 0;
        int portalAzar;
        for (int i = Integer.parseInt(jugadores[turno][2]) + 1; i < tipoCasilla.length - 1; i++) {
            if (String.valueOf(tipoCasilla[i]).equals("p")) {
                cantidadPortal++;
            }
        }
       portalAzar = (int) (Math.random() * cantidadPortal);
        cantidadPortal = 0;
        for (int i = Integer.parseInt(jugadores[2][turno]) + 1; i < tipoCasilla.length - 1; i++) {
            if (String.valueOf(tipoCasilla[i]).equals("p")) {
                cantidadPortal++;
            }
